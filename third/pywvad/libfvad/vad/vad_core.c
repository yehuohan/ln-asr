...
#if 1
    overhead1 = self->over_hang_max_1;
    overhead2 = self->over_hang_max_2;
    individualTest = self->individual;
    totalTest = self->total;
#else
  if (frame_length == 80) {
...
#if 1
int WebRtcVad_set_mode_core(VadInstT* self,
        int16_t ohmax1, int16_t ohmax2, int16_t lthr, int16_t gthr) {
    self->over_hang_max_1 = ohmax1;
    self->over_hang_max_2 = ohmax2;
    self->individual = lthr;
    self->total = gthr;
    return 0;
}
#else
