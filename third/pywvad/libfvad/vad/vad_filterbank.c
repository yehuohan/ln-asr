...
static void HighPassFilter(const int16_t* data_in, size_t data_length,
  ...
  //
  // loop {
  //    y(n) = (b0 * x(n) + b1 * s0 + b2 * s1) - (a1 * s2 + a2 * s3)
  //    s1 = s0
  //    s0 = x(n)
  //    s3 = s2
  //    s2 = y(n)
...
static void AllPassFilter(const int16_t* data_in, size_t data_length,
  ...
  // 计算式的单位为Q15
  // 对x(n)进行2倍下采样，再进行二阶全通滤波
  // n = 0, i = 0
  // loop {
  //    y(n) = (s + c * x(i)) / 2   : Q15/Q16 = Q(-1) = 1/2
  //    s = x(i) - (c * y(n)) * 2   : Q15/Q14 = Q1 = 2
  //    n += 1
  //    i += 2
  // }
  // save(s)

  // => y(n) = [x(n-2) - c * y(n-2) * 2 + c * x(n)]/2
  // => H(z) = 1/2 * (c + z^(-2)) / (1 + c * z^(-2))

  for (i = 0; i < data_length; i++) {
    tmp32 = state32 + filter_coefficient * *data_in;  // Q15
    tmp16 = (int16_t) (tmp32 >> 16);  // Q(-1)
    *data_out++ = tmp16;
    state32 = (*data_in * (1 << 14)) - filter_coefficient * tmp16;  // Q14
    state32 *= 2;  // Q15.
    // 等价于 state32 = (*data_in * (1 << 15)) - filter_coefficient * tmp16 * 2;  // Q15
    // data_in先转到Q15
    // filter_coefficient为Q15
    // tmp16为Q(-1)，需要乘2转到Q0，这样(filter_coefficient*tmp16*2)就为Q15了
    data_in += 2;
  }
...
static void SplitFilter(const int16_t* data_in, size_t data_length,
  ...
  // A0(z): AllPassFilter(c = 0.64)
  // A1(z): AllPassFilter(c = 0.17)
